...
excerpt: "An effort to drive the community into talking about the responsible disclosure was met by strong criticism."
author: [arnav_arora, andrey_kurenkov]
tags: [OpenAI,hype]
...
## What Happened
On February 14 of 2019 [OpenAI](https://en.wikipedia.org/wiki/OpenAI) [^OpenAI] --
released the blog post [Better Language Models and Their Implications](https://blog.openai.com/better-language-models/), which
revealed a scaled up version of their transformer-based
language model [^technical] initially released in June 2018. The norm in
AI research when displaying new findings is to openly share your
results and often also the dataset , the code and pre-trained models
for running those experiments. This has been one of the fundamental
reasons the field has progressed so rapidly over the past decade, as
researchers don’t have to spend months reproducing other people’s work
and can verify and build on results quickly. OpenAI, in the past, has
conformed to this norm. However, this time they decided to release
only a smaller sample of their model, a paper with some of the
technical details and a blog post highlighting some possible adverse
implications of the model along with samples of the output. This was
done as part of a larger effort to promote discussion around dual use [^dual_use] of these ever-improving algorithms.

[^OpenAI]: A non-profit research company with a mission to develop safe Artificial General Intelligence in the long run
[^technical]: A transformer is a popular new idea for machine learning with language -- read more [here](http://jalammar.github.io/illustrated-transformer/). A language model is an AI meant to injest sequences of text and predict what words are most likely to occur next
...
Around the same time as OpenAI made the announcement, a number of
publications released articles about GPT-2 and OpenAI’s decision to not
open source it. Many of them were provided with limited access to the
model for a short period of time before the release -- before fellow AI 
researchers wre made aware of it. This led to a number of pieces with bombastic 
headlines and overblown claims of how the end is nigh because AI can now generate 
human like text, which was nowhere close to reality.
...
As is often the case, most reporting from reputable sources covered the details accurately despite sometimes having hyperbolic headlines. For example, from
...
> “In the wrong hands, GPT-2 could be an automated trolling machine,
spitting out endless bile and hatred. If it becomes more sophisticated
and able to persuade and convince in a reliable fashion, it could cause
even subtler damage, influencing debate online. Countries like Russia
and Saudi Arabia, which already employ thousands of online propagandists
to abuse government opponents and push official talking points, could
scale up their efforts overnight. And remember, none of the text GPT-2
produces is copied and pasted: it’s all newly generated, thus harder to
filter and more easily shaped to specific ends.” 
...
out about new results in their field through the media instead of
conference proceedings struck a chord with the research community with
doubts about OpenAI’s ulterior motives. The decision of not publicly
releasing their models for public safety was also widely discussed. Some
lauded the move as a bold and necessary step towards responsible AI:
...
Others considered it either a futile attempt or a poor decision doing more harm to reproducibility and openness than good:
...
The conversation was prolonged enough for many to write more in-depth think pieces summarizing their stance:
...
In summary, while most people recognised the need for talking about the
...
Journalists after all are not the experts on the topic and it’s hard to put such an achievement into context while being an outsider. Having said that, these technologies affect everybody and not just the NLP or ML community. This is a hard conversation that needs to happen. 

Some are trying to take this in good faith and promote conversation around this important topic. 

<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">What have been your favorite *on-the-merits* *pro-release* OpenAI GPT-2 takes (on twitter or elsewhere)?<br><br>I&#39;m looking for clear good-faith explanation of the pro-release (or anti-media-attention?) position right now, not clever snark.</p>&mdash; Catherine Olsson (@catherineols) <a href="https://twitter.com/catherineols/status/1097203255136796672?ref_src=twsrc%5Etfw">February 17, 2019</a></blockquote>
<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
...
Jack Clark and Miles Brundage at OpenAI have been very active in trying to provide the answers to the numerous questions around this, share the thought process surrounding their decision and take feedback on how they can improve things in the future.
...
They’ve also hinted at a possible slow release in the coming months with
some more insights into the policy and ethics research that went into
this decision. This is a necessary step in taking this conversation
forward and to show that they’re serious about responsible AI research.

Lessons can be learnt from the [Information security or Biology](http://blog.geomblog.org/2019/02/openai-ai-threats-and-norm-building-for.html)
landscape where protocols and best practices for such events already
exist. We need systems in place to determine the [threat models](https://aivillage.org/blog/2019/2/23/gpt-2-and-threatmodels) of
these algorithms and then make decisions on what to do next.

## TLDR

Whatever the motives of OpenAI may be, we need to recognise that these
advanced technologies are bigger than us and affect a lot of people.
Hence, discussion about its implications is an area that needs to get
more attention and should include a diverse set of people.
