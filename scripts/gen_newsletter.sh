...
git checkout master; git pull; git checkout -b digest_$digest_num
python csv2md.py -n=$digest_num

# Push changes

rm *.csv
git add ../_posts/digests/* ../assets/img/digests/$digest_num/*
git commit -am $digest_name; git push --set-upstream origin $digest_name
